...
class MCPClient {
    ...
    async connectToServer(serverScriptPath, venvPath) {
        try {
            ...
            console.log(`Connected to server with tools (${this.provider}):`, this.tools.map(({ name }) => name));
        }
        catch (e) {
...
>>>>>>> main
    ...
    async chatLoop() {
        ...
        try {
            ...
            console.log("Type your queries, 'switch' to change provider, or 'quit' to exit.");
            while (true) {
...
async function main() {
    if (process.argv.length < 3) {
        console.log("Usage:");
        console.log("  node index.js <path_to_server_script> [venv_path] [provider]");
        console.log("");
        ...
        console.log("  node build/index.js ../mcp-python/server/test_server.py ../mcp-python/.venv");
        console.log("  node build/index.js ../mcp-python/server/test_server.py ../mcp-python/.venv openai");
        console.log('  node build/index.js ../mcp-python/server/test_server.py "" claude');
        return;
    }
    const serverPath = process.argv[2];
    const venvPath = process.argv[3] && process.argv[3] !== "" ? process.argv[3] : undefined;
    const provider = process.argv[4] || "openai";
